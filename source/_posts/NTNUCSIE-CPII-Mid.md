...
第二題是要處理字串壓縮，這題我把字元對應的二進製讀進來，並用 strtol 來轉成十進位再丟到開好大小為 29 的 array 裡，然後同時儲存長度來處理開頭是 0 的情況。接下來就是開一個 byte 暫存然後計算暫存長度跟對應到字元的長度是否大於等於 8，如果是就把暫存的 byte 寫入檔案，然後把暫存的 byte 清空，直到該字元加字串的長度小於 8，中間合併字串就只是單純的位元運算，然後等把字串全部寫完後，再把處理最後的 padding。
